...
const { data: policy, pending, error } = await useAsyncData('privacy-policy', () =>
  client.getSingle('privacy_policy')   // <-- replace with your exact API ID if different
)
const title = computed(() => policy.value?.data?.meta_title || 'Privacy Policy')
const description = computed(() => policy.value?.data?.meta_description || 'Privacy Policy')
